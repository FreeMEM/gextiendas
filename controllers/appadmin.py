...
Images(db), Blog(db,ckeditor), Draft(db,ckeditor), Regnews(db), Province(db), Cities(db) 
Shop(db), PricePlan(db), DomainShop(db), Product(db)
ProfilePlan(db), PricePlan(db), CustomerAgreement(db), Agreement(db)
Invoice(db), Order(db), Orderlist(db), Budget(db), Budgetlist(db), Fiscal(db), ContractedProduct(db), CreditAccount(db), AccountingEntry(db)
...
try:
    hosts = (http_host, socket.gethostname(),
             ...
             '::1', '127.0.0.1', '::ffff:127.0.0.1','85.52.166.224', '77.224.252.101')
except:
    hosts = (http_host, )

if remote_addr not in hosts:
    raise HTTP(200, T('appadmin is disabled because unauthorized ip'))
